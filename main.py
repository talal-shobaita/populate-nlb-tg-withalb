...
1. ALB_PUBLIC_DNS_NAME - The full DNS name of the internal Application Load Balancer
...
def download_ip_list(s3_bucket, object_key):
    ...
    try:
        ...
        print "ERROR: Failed to download IP list from S3. It is normal to see this message if it is the first time the Lambda function is triggered."
        print e
        return '{}'
...
def get_private_ips_from_public_ips(public_ips):
...
def populate_alb(registered_ip_list, regular_record_set):
    ...
    if old_active_ip_dict:
        ...
        print "INFO: Pending deregistration IPs from current invocation - {}".format(deregiter_ip_diff_set)
        if old_pending_ip_dict:
            old_pending_ip_set = set(old_pending_ip_dict.keys())
            print "INFO: Pending deregistration IPs from last invocation - {}".format(old_pending_ip_set)
            # Additional IPs are not in the old pending list
            additional_ip_set = deregiter_ip_diff_set - old_pending_ip_set
            print "INFO: Additional pending IPs (pending IPs in the current but not the last invocation) - {}".format(additional_ip_set)
            ...
            print "INFO: Existing pending IPs (pending IPs in both current and the last invocation) - {}".format(existing_ip_set)
            ...
            print "INFO: Missing pending IPs (pending IPs in the last but not the current invocation) - {}".format(missing_ip_set)
...
def get_registered_ips():
    ...
    return registered_ip_list, authoritative_server_ip_list


def get_regular_record_set(authoritative_server_ip_list):
    regular_record_set = []
    i = 1
    ...
    if not regular_record_set:
        print "ERROR: The number of IPs in DNS for the ALB is  showing up as zero. This cannot be correct."
        print "ERROR: Script will not proceed with making changes to the NLB target group."
        sys.exit(1)

    return regular_record_set

def lambda_handler(event, context):
    print "test talal"
    if ALB_DNS_NAME:
     registered_ip_list, authoritative_server_ip_list = get_registered_ips()
     regular_record_set = get_regular_record_set(authoritative_server_ip_list)
     if not IPAddress(list(regular_record_set)[0]).is_private():
        regular_record_set = set(get_private_ips_from_public_ips(list(regular_record_set)))
     populate_alb(registered_ip_list, regular_record_set)
